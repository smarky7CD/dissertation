\begin{figure}[thp]
    %	\Wider[4em]{
            ...
            \end{pchstack}
    %	}
      \caption{Keyed count-min sketch structure $\mathrm{CMS}[R,m,k]$ admitting point queries for any~$x \in \univ$. The parameters are integers $m,k \geq 0$, and a keyed function $R: \keys\by\univ \to [m]^k$ that maps data-object elements (encoded as strings) to a vector of positions in the array~$\v.M$. A concrete scheme is given by a particular choice of
      parameters.
      } 
      \label{fig:cms}
    \end{figure}
