...
%Further, while the syntax from~\cite{clayton2019probabilistic} that we present above is highly expressive, it may not capture all data structures one may want to examine. For instance, an \emph{authenticated data structure}~\cite{tamassia2003authenticated} may require some changed to the functional definitions, as well as the addition of a \emph{verification function}~$\schemefont{VFY}$ that would act to verify proofs to query responses. Lastly, we note that the syntax does not support describing (nor is it clear how to easily modify the syntax to be able to support) what could be viewed as distributed data structures, such as blockchains and distributed hash tables. This is not the focus of this work, and we point to the reader~\cite{zhang2019security} and~\cite{needels2009secure}, respectively, if interested in such areas.
